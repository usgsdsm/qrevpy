...
class PreMeasurement(object):
    ...
    def pt3_data(self):
        ...
        # Sin match
        sin_match = re.findall('((Sin|SIN).*?(Cos|COS))', self.data, re.MULTILINE | re.DOTALL)[0][0]
        sin_array = np.array(re.findall('\d+\.*\d*', sin_match))

        # Cos match
        cos_match = re.findall('((Cos|COS).*?RSSI)', self.data, re.MULTILINE | re.DOTALL)[0][0]
        ...
        for rssi_match in rssi_matches:
            rssi_numbers.append(re.findall('\d+\.*\d*', rssi_match))
        rssi_array = np.array(rssi_numbers[0])
        ...
        for n, lag_match in enumerate(lag_matches):
            ...
            corr_data = np.array(numbers[(bm_count * 4)::]).reshape([8, (bm_count * 4) + 1])[:, 1::]
            ...
            # 8 tests arranged in sets of 2. The linear is 1st followed by the hard limit.
                ...
                # Linear wide bandwidth (n=0)
                if n == 0:

                    pt3['linear']['high_wide']['corr_table'] = corr_data[:, 0:4]
                    pt3['linear']['high_wide']['noise_floor'] = rssi_array[n * 4: (n + 1) * 4]

                    pt3['linear']['low_wide']['corr_table'] = corr_data[:, 4::]
                    pt3['hard_limit']['low_wide']['noise_floor'] = rssi_array[(n + 1) * 4: (n + 2) * 4]

                # Linear narrow bandwidth (n=1)
                elif n == 1:

                    pt3['linear']['high_narrow']['corr_table'] = corr_data[:, 0:4]
                    pt3['linear']['high_narrow']['noise_floor'] = rssi_array[(n + 1) * 4: (n + 2) * 4]

                    pt3['linear']['low_narrow']['corr_table'] = corr_data[:, 4::]
                    pt3['linear']['low_narrow']['noise_floor'] = rssi_array[(n + 2) * 4: (n + 3) * 4]

                # Hard limited wide bandwidth (n=2)
                elif n == 2:

                    pt3['hard_limit']['high_wide']['corr_table'] = corr_data[:, 0:4]
                    pt3['hard_limit']['high_wide']['sdc'] = sin_array[(n - 2) * 4: (n - 1) * 4]
                    pt3['hard_limit']['high_wide']['cdc'] = cos_array[(n - 2) * 4: (n - 1) * 4]
                    pt3['hard_limit']['high_wide']['noise_floor'] = rssi_array[(n + 2) * 4: (n + 3) * 4]

                    pt3['hard_limit']['low_wide']['corr_table'] = corr_data[:, 4::]
                    pt3['hard_limit']['low_wide']['sdc'] = sin_array[(n - 1) * 4: n * 4]
                    pt3['hard_limit']['low_wide']['cdc'] = cos_array[(n - 1) * 4: n * 4]
                    pt3['hard_limit']['low_wide']['noise_floor'] = rssi_array[(n + 3) * 4: (n + 4) * 4]

                # Hard limited narrow bandwidth (n=3)
                elif n == 3:

                    pt3['hard_limit']['high_narrow']['corr_table'] = corr_data[:, 0:4]
                    pt3['hard_limit']['high_narrow']['sdc'] = sin_array[(n - 1) * 4: n * 4]
                    pt3['hard_limit']['high_narrow']['cdc'] = cos_array[(n - 1) * 4: n * 4]
                    pt3['hard_limit']['high_narrow']['noise_floor'] = rssi_array[(n + 3) * 4: (n + 4) * 4]

                    pt3['hard_limit']['low_narrow']['corr_table'] = corr_data[:, 4::]
                    pt3['hard_limit']['low_narrow']['sdc'] = sin_array[n * 4: (n + 1) * 4]
                    pt3['hard_limit']['low_narrow']['cdc'] = cos_array[n * 4: (n + 1) * 4]
                    pt3['hard_limit']['low_narrow']['noise_floor'] = rssi_array[(n + 4) * 4: (n + 5) * 4]
            ...
            # 8 tests in groups of 4. Hard limit is the first group then the linear.
                ...
                # Linear data (n=1)
                    ...
                    pt3['linear']['high_narrow']['noise_floor'] = rssi_array[(n + 4) * 4: (n + 5) * 4]
                    ...
                    pt3['linear']['high_narrow']['noise_floor'] = rssi_array[(n + 6) * 4: (n + 7) * 4]
        self.result['pt3'] = pt3
