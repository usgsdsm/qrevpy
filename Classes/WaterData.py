...
class WaterData(object):
            ...
            if new_sys - orig_sys > 0:
                ...
                for ii in range(n_ens):
                    ...
                    if len(col_idx) > 0:
                        ...
                        #Get velocity data
                        vel_raw = np.squeeze(self.__raw_vel_mps[:,ii,:]).func_defaults
                        temp_THPR = hpr_matrix * vel_raw[:3,:]
                        temp_THPR[3,:] = vel_raw[3,:]
    ...
    def set_nav_reference(self, boat_vel):           
        ...
        if len(boat_vel[boat_vel.selected]) > 0:
            self.__u_mps = np.add(self.__u_earth_no_ref_mps, boat_vel[boat_vel.selected]['u_processed_mps'])
            self.__v_mps = np.add(self.__v_earth_no_ref_mps, boat_vel[boat_vel.selected]['v_processed_mps'])     
            self.nav_ref = boat_vel[boat_vel.selected]['nav_ref']
        else:
        ...
        self.valid_data[1:] = np.tile(self.valid_data[1], [7,1,1])
    ...
    def apply_interpolation(self, transect, kargs=None):           
        self.__u_processed_mps = np.repeat(np.nan, len(self.__u_mps))
        self.__v_processed_mps = np.repeat(np.nan, len(self.__v_mps))
    ...
    def apply_filter(self, transect, kargs = None):
        ...
        if kargs is not None:
            ...
            self.filter_excluded(transect, self.excluded_dist)
    ...
    def __filter_beam(self, setting):
        ...
        if self.beam_filter > 0:
            ...
            #Find invalid raw data
            valid_vel =  [self.__cells_above_sl] * 4
            valid_vel[np.isnan(self.__raw_vel_mps)] = 0
